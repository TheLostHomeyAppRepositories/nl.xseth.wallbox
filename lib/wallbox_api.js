  ...
  */
  async authenticate() {
    ...
    if(this._jwt){
      ...
      if(Math.round(this._jwtTokenTtl / 1000) > Math.floor(Date.now() / 1000)){
        this.homey.log('Valid auth!');
        return
      }

      // Need to refresh token, if refresh token is still valid
      if (this._jwtRefreshToken && Math.round(this._jwtRefreshTokenTtl / 1000) > Math.floor(Date.now() / 1000)){
        this.homey.log('Refreshing auth!', data);
