  ...
  */
  async authenticate() {
    ...
    if(this._jwt){
      ...
      if(Math.round(self._jwtTokenTtl / 1000) > Math.floor(Date.now() / 1000))
        return

      // Need to refresh token, if refresh token is still valid
      if (this._jwtRefreshToken && Math.round(self._jwtRefreshTokenTtl / 1000) > Math.floor(Date.now() / 1000)){
